...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const containerModeloWord = document.getElementById('containerModeloWord');
    ...
    function createRecordCard(item, index) {
        ...
        for (const key of allKeys) {
            if (!hiddenFields.has(key)) {
                ...
                if (typeof value === 'string' && value.startsWith('http')) {
                    ...
                    const extensionMatch = trimmedLink.match(/\.(\w+)(\?|$)/);
                    const extension = extensionMatch ? extensionMatch[1].toLowerCase() : 'desconhecida';
                    ...
                    if (isImage) {
                        ...
                        linksHtml += `
                            <img src="${trimmedLink}" title="Visualizar Imagem" class="viewer-image" style="display:none;" alt="Documento" />
                            <button type="button" class="document-link-button ${isOpened ? 'document-link-opened' : ''}" data-img="${trimmedLink}"><i class="fas fa-image"></i> Visualizar Imagem</button>
                        `;
                    } else 
                    if (extension === 'pdf') {
                        viewerUrl = `${pdfjsViewerUrl}?file=${encodeURIComponent(trimmedLink)}`;
        ...
        const decisionCardHtml = `
            <div class="decision-card">
                ${decision.status ? `
                    ...
                    <button class="nav-btn clear-decision-btn" data-id="${uniqueId}">Decidir Novamente</button>
                ` : `
        ...
        setTimeout(() => {
        const imageBtns = cardWrapper.querySelectorAll('button.document-link-button[data-img]');
        imageBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                // Inicializa o Viewer apenas para esta imagem
    ...
    function addDocumentLinkListeners(card) {
        ...
        documentLinks.forEach(link => {
            link.addEventListener('click', () => {
                ...
                const url_img = event.currentTarget.getAttribute('data-img');
                if (!openedDocuments.has(url)) {
                    openedDocuments.add(url);
                    saveOpenedDocuments();
                    link.classList.add('document-link-opened');
                }else
                if (!openedDocuments.has(url_img)) {
                    openedDocuments.add(url_img);
                    saveOpenedDocuments();
                    link.classList.add('document-link-opened');
                }
    ...
    function updateStatusBox() {
        ...
        if (checkIfAllDecided()) {
            ...
            containerModeloWord.style.display =  'flex';
            containerModeloWord.style.flexDirection =  'column';
            containerModeloWord.style.alignItems =  'center';
            ...
            containerModeloWord.style.display = 'none';
        }
    ...
    function renderReportRecursiveHtml(data, keys, level = 0) {
        ...
        for (const groupKey in data) {
            if (data.hasOwnProperty(groupKey)) {
                ...
                if(cleanedKey !== 'Grupo'){
                html += `<h${level + 2} style="font-family: 'Poppins', sans-serif; text-align: center; font-size: ${18 - (level * 2)}px; font-weight: 600; margin-top: 20px; margin-bottom: 10px; color: #2c3e50;">${groupKey}</h${level + 2}>`;
                html += '<hr style="border: 0; height: 1px; background-color: #e0e0e0; margin-bottom: 15px;">';
                }

                if (Array.isArray(subgroup)) {
                    const separarDeferidas = document.getElementById('separarDeferidasCheckbox').checked;
                    let tableRows = '';
    ...
    function generateWordReport() {
        ...
        if (separarDeferidas) {
            // Separar por deferidas e não deferidas
            const deferidas = [];
            const naoDeferidas = [];
            processedData.forEach(item => {
                const rawUniqueId = item['CPF:'] || item['E-mail:'];
                ...
                if (decision && decision.status === 'deferida') {
                    ...
                    naoDeferidas.push(item);
                }
            });

            if (deferidas.length > 0) {
            reportContent += `<div style="font-family: 'Poppins' , sans-serif; text-align: center; margin-bottom: 10px; margin-top: 10px;">`;
            reportContent += `<h3 style="#2c3e50;">Candidatos com Inscrições Homologadas</h3>`;
            reportContent += '<hr style="height: 1px; background-color: #e0e0e0;">';
            reportContent += `</div>`;
            reportContent += renderReportRecursiveHtml(getGroupedData(deferidas), groupingKeys);
            reportContent += '<hr style="height: 1px; background-color: #e0e0e0;">';
            }

            if (naoDeferidas.length > 0) {
            reportContent += `<div style="font-family: 'Poppins' , sans-serif; text-align: center; margin-bottom: 10px; margin-top: 10px;">`;
            reportContent += `<h3 style="color:#2c3e50;">Candidatos/as com inscrições não Homologadas</h3>`;
            reportContent += '<hr style="height: 1px; background-color: #e0e0e0;">';
            reportContent += `</div>`;
            reportContent += renderReportRecursiveHtml(getGroupedData(naoDeferidas), groupingKeys);
            reportContent += '<hr style="height: 1px; background-color: #e0e0e0;">';
            }

        } else {
            // Relatório normal
            reportContent = renderReportRecursiveHtml(groupedData, groupingKeys);
        }
        ...
        const reportHeaderHtml = `
            ...
            <div style="font-family: 'Poppins', sans-serif; text-align: center; margin-bottom: 10px;">
                ...
                <h3 style="font-size: 16px; font-weight: 700; color: #2c3e50;">HOMOLOGAÇÃO DAS INSCRIÇÕES</h3>    
            </div>
                <p style="font-size: 16px; color: #2c3e50; text-align:justify;">A Comissão de Seleção torna público o resultado da homologação das inscrições do Processo Seletivo Simplificado para ${processoSeletivo} da SEAD/UFBA, estabelecido no âmbito do Sistema Universidade Aberta do Brasil.</p>
        `;
        ...
        // Converte para blob DOCX
        const blob = window.htmlDocx.asBlob(conteudoHTML, { orientation: "portrait" });

        // Baixa o arquivo
        saveAs(
            blob,
            `Relatorio_Homologacao_${new Date().toLocaleDateString("pt-BR").replace(/\//g, "-")}.docx`
        );
